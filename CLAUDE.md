...
- ~~仓库无 `pytest.ini` / `pyproject.toml [tool.pytest]`，跑测试依赖默认发现。~~ **已修复**：`pyproject.toml` 添加 `[tool.pytest.ini_options]`（testpaths + asyncio_mode）。
- ~~无 `black` / `ruff` / `mypy` 配置文件——依赖全局 `~/.claude/rules/coding-style.md` 的默认值。~~ **已修复**：`pyproject.toml` 添加 `[tool.ruff]`（target-version=py310, line-length=100, select E/F/W/I）+ `[tool.mypy]`（ignore_missing_imports, warn_return_any）。
...
- ~~`docs/agents/domain.md` 规定仓库根目录应有一个 `CONTEXT.md`，目前不存在——本 CLAUDE.md 暂时兼任 root context；待 domain docs 成熟再单独拆出。~~ **已完成**：`CONTEXT.md` 创建于仓库根目录，含 domain glossary（12 术语）、architecture layers、key invariants。
- ~~仓库暂无 `.github/workflows/` CI，Loop Workflow 里的"push only after green"当前以**本地** `pytest` + `npm run build` 为准。~~ **已完成（2026-05-17）**：`.github/workflows/ci.yml` 创建，含 3 个 job：`test`（Python 3.10/3.11/3.12 矩阵，跳过需 CastEngine 的深度测试）、`frontend`（Node 20 + npm ci + build）、`lint`（ruff check）。
- ~~**`docs/test-plan-opencode-glm-castengine.md` 未落盘**（2026-05-13）：7-entry E2E 已跑出实测遥测（34 RepairLog、35 LLM 边、100% reasoning_summary 非空、GLM-5 在 3/7 source 上 0-edge 放弃的模式），但 opencode + GLM-5 + CastEngine 的全量测试计划文档尚未成文。~~ **已完成（2026-05-17）**：`docs/test-plan-opencode-glm-castengine.md` 落盘，含 5 类断言（reasoning 命中率、放弃率分布、mirror 吞吐、门禁通过率、审计字段完整性）+ 7-entry 和 3-entry 实测基线 + 已知问题 + 下一步。
- **35 LLM 边 vs 34 RepairLog 的 1 条差值**（2026-05-13，7-entry run 观察到）：`65cecfa5cf3a→f41875af7db5 @line132` 与 `94a77ea67585→1ccd8bba9eab @line1189` 两条 CALLS 边在 Neo4j 里没有 `(caller_id, callee_id, call_location)` 三元组精确匹配的 RepairLog。根因已定位：`call_file` 路径格式不一致（相对 vs 绝对路径）导致 `call_location` 字符串不匹配。回归测试已添加（`test_write_edge_repair_log_call_location_matches_edge`）。修复方案：在 `write_edge` 中对 `call_file` 做路径规范化（strip target_dir prefix），但需要 E2E 验证。
- **门禁机制尚未在真实 E2E 中 pass 过**（2026-05-13）：`check-complete` TypeError 修复前每次必 False；修复后的 7-entry run 所有 source 都有剩余 pending GAP（0-edge 放弃 + counter-example 全覆盖），没跑出过 `complete=True` 的实际路径。subprocess JSON 契约已有单元测试覆盖（`test_gate_complete_true_subprocess` + `test_gate_subprocess_json_contract`），但真实 Neo4j + agent 全链路尚未验证——需要挑一个 agent 能真正解全 GAP 的 source 或构造用例。
- ~~仓库根目录没有 check-in 的 `config.yaml`，4 个 CLI 子命令默认 `--config config.yaml` 找当前工作目录——首次跑前要么 `cp codemap_lite/config/default_config.yaml config.yaml`，要么显式 `--config codemap_lite/config/default_config.yaml`。~~ **已解决**：`config.yaml` 已 check-in 到仓库根目录（含 CastEngine target_dir + opencode 配置）。注意：该文件含 API key，生产部署应改用 `${VAR}` 环境变量注入。
- ~~**FeedbackStore 反例泛化去重（architecture.md §3 反馈机制 step 4 "相似 → 总结合并"）** 当前实现是**精确 pattern 字串匹配**（`FeedbackStore.add()` 按 `pattern` 精确比对决定是否 dedup），而架构要求**LLM 语义相似度判断**后合并+泛化。~~ **部分完成（2026-05-17）**：添加两层 dedup：(1) 精确匹配（原有）；(2) 模糊匹配——`_normalize_pattern()` 去除行号和文件路径后用 Jaccard token similarity ≥ 0.7 判定相似。覆盖了"同一 bug 模式在不同位置"的 80% 场景。**仍缺**：LLM 语义相似度判断 + 泛化摘要生成（需要调用 LLM API，属于 Phase 2 增强）。
