...
- 2026-05-13 · agent+docs: `write-edge` CLI 管道化 `--llm-response` + `--reasoning-summary`（Tick #33 inspector 的字面喂料）——Tick #33 把 `EdgeLlmInspector` 接到 `api.getRepairLogs()`，但**生产路径里每条 llm 边的 `RepairLogNode.llm_response` / `reasoning_summary` 都是空串**：`agent/icsl_tools.py` 的 `write-edge` argparse 只有 `--caller/--callee/--call-type/--call-file/--call-line` 五个 flag，`write_edge()` in-process 函数虽然接受 `llm_response` + `reasoning_summary` kwarg 但 CLI 路径强制传空——审阅者点 ★ 边只看到 "No reasoning summary recorded"，Tick #32 + #33 搭的整条 audit 链在真实 agent flow 里等于空响。本 tick 把"主捕获路径"从 post-hoc `.jsonl` 提取翻转为"agent 在 `write-edge` 时同步前传"，按 architecture-first 顺序：(1) `architecture.md §3 Agent 内循环 step 2d` write-edge 签名扩成 `[--llm-response <raw>] [--reasoning-summary <one-sentence justification>]`，明确 llm 路径**应当传**、静态/symbol_table 路径合理留空默认空串；§3 推理捕获第 3 点改写为"主路径 = Agent 同步前传 → `EdgeLlmInspector` 直接读；Fallback = `.jsonl` 事后提取只作兜底"。(2) `codemap_lite/agent/icsl_tools.py` write-edge subparser 加两个 argparse flag（default=""），main() dispatch 分支把 `args.llm_response` + `args.reasoning_summary` 转发给 `write_edge()`；module docstring CLI 例子同步更新。(3) `codemap_lite/agent/claude_md_template.py` write-edge 工具行描述加两个可选 flag + 新增 "Reasoning capture" 子节，明确告诉 agent "Every write-edge call you make here is an **llm-resolution** — pass both on every edge you resolve here"，留空会在 UI 显示 "No reasoning summary recorded" 强制审阅者 fallback 到 log 文件。(4) 测试：`test_cli_write_edge_creates_calls_edge` 加 backwards-compat guard（省略 flag 仍产 RepairLogNode 且 reasoning 字段为空）；新增 `test_cli_write_edge_forwards_llm_response_and_reasoning`（传入 `--llm-response "ptr is assigned a DerivedHandler..."` + `--reasoning-summary "picked DerivedHandler::handle..."` → `store.repair_logs[-1].llm_response` / `.reasoning_summary` 精确命中）。对齐 architecture.md §3 Agent 内循环 step 2d + §3 推理捕获 + §4 RepairLogNode schema + §5 RepairLog inspector 契约 + ADR #51 三元组定位；兑现北极星指标 #2（调用链可信度可见性——llm 边从"可解释（UI 渲染契约已通）"升级为"**有料可读**（生产 agent 每条边都带真实推理落库）"，闭合 Tick #32→#33→本 tick 三段审计链）+ #5（状态透明度——`RepairLogNode.reasoning_summary` 不再恒空）；pytest 154/154 绿（153 + 1 新增），`npm run build` 绿（5.16s），`python -m codemap_lite.agent.icsl_tools write-edge --help` 双 flag 可见。
---
